# Migrating Expo UI to FindFit Design System

Read `artifacts/findfit-ds/docs/AGENTS.md` and
`artifacts/findfit-ds/docs/consuming-expo.md` first. Use this guide
...
- Rewrite color/theme imports to
  `@workspace/findfit-ds/lib/native-theme` and
  `@workspace/findfit-ds/hooks/use-colors`.
- Rewrite the root layout to use
  `@workspace/findfit-ds/hooks/use-fonts` while preserving its
  SplashScreen gating.
...
and hook directly from `@workspace/findfit-ds` and retains only
